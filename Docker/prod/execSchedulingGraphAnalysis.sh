...
echo "docker run --rm --name schedulingGraphAnalysisExec -a stdin -a stdout -v \"$srcDir\":\"$srcDir\": scheduling_graph_analysis:1.0 bash -c \"useradd -d $homeDir $user; sudo -H -u $user bash -c \"cd $/working; python3 /srv/SchedulingGraphAnalysis/src/FindCycles.py $cmd\"\""

docker run --rm --name schedulingGraphAnalysisExec -a stdin -a stdout -it -v "$srcDir":/working: scheduling_graph_analysis:1.0 bash -c "useradd -d $homeDir $user; sudo -H -u $user bash -c \"cd /working; python3 /srv/SchedulingGraphAnalysis/src/FindCycles.py $cmd\""